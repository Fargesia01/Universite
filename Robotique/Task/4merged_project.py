...
data.write('step,state,x_center,y_center,width,height,conf,label,ground_left,ground_middle,ground_right,ds_pid,ps0,ps1,ps2,ps3,ps4,ps5,ps6,ps7\n')
...
########################################################################################
### PHASE 1 Initialisation: initiate lists to track detections and states
########################################################################################
...
########################################################################################
### PHASE 2 Initialisation: initiate lists to track detections and states
########################################################################################
...
########################################################################################
### PHASE 4 Initialisation: initiate PID values and several distance measurements
########################################################################################
...
########################################################################################
####### Main program: while-Loop running through all the states (running_phase) 1-4  ###
########################################################################################
...
while robot.go_on():
	...
	if (running_phase == 2):
		...
		if (not firstVioletLine):
			...
			# Log ground data
			logGROUND(data, "FORWARD", stepcounter, gs)
	...
	if (running_phase == 3 and turn < 3):
        ...
        # Log prox data
		logPROX(data, "TURNING", stepcounter, ps)
	...
	if (running_phase == 4):
		...
		if(cornerReached < 3):
			...
			if (cornerReached == 0):
				...
				# Log wallfollowing data (right)
				logPID(data, "WALLFOLLOWER_RIGHT", stepcounter, ds, ps)
			...
			if (cornerReached == 2):
                ...
                # Log wallfollowing data (left)
				logPID(data, "WALLFOLLOWER_LEFT", stepcounter, ds, ps)
        ...
        # Count stepcounter for plotting functions
		stepcounter += 1
